...
class Sensor:
    ...
    def init(self):
        # Esperar como máximo 200 ms a que el bit FRESH_OUT_OF_RESET sea 1
        for _ in range(40):                   # 40 × 5 ms  ≈ 200 ms
            if self.myRead16(0x0016) & 0x01:
                break
            time.sleep_ms(5)
        # Borrar el bit (escribir 0) para indicar que el host lo ha leído
